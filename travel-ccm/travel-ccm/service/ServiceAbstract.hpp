#ifndef __TRAVEL_CCM_SERVICEABSTRACT_HPP
#define __TRAVEL_CCM_SERVICEABSTRACT_HPP
...
operator<< (std::basic_ostream<charT, traits>& ioOut,
            const TRAVEL_CCM::ServiceAbstract& iService) {
  /**
     string stream:
      - with same format
...
operator>> (std::basic_istream<charT, traits>& ioIn,
            TRAVEL_CCM::ServiceAbstract& ioService) {
  // Fill Service object with input stream
  ioService.fromStream (ioIn);
  return ioIn;
}

#endif // __TRAVEL_CCM_SERVICEABSTRACT_HPP
