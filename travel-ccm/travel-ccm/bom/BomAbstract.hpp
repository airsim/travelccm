#ifndef __TRAVEL_CCM_BOM_BOMABSTRACT_HPP
#define __TRAVEL_CCM_BOM_BOMABSTRACT_HPP
...
operator<< (std::basic_ostream<charT, traits>& ioOut,
            const TRAVEL_CCM::BomAbstract& iBom) {
  /**
     string stream:
      - with same format
...
operator>> (std::basic_istream<charT, traits>& ioIn,
            TRAVEL_CCM::BomAbstract& ioBom) {
  // Fill Bom object with input stream
  ioBom.fromStream (ioIn);
  return ioIn;
}

#endif // __TRAVEL_CCM_BOM_BOMABSTRACT_HPP
